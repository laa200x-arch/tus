...
按用户提供的移动端参考图进行高保真复刻。参考图是 iOS 首页的强视觉约束，信息顺序、卡片比例、圆角、留白、阴影、色彩层级、字体层级、小能仔 Hero 位置和主要交互入口必须与参考图一致；仅排除系统状态栏、设备边框等非应用内容，并将参考图中的示例数据替换为真实聚合接口数据。
...
Windows 继承参考图的卡片语言、柔和紫色视觉、信息层级和小能仔表现，但不得简单横向拉伸或复制手机底部导航。Windows 保留左侧导航和顶部栏，首页调整为三层宽屏 Dashboard：
